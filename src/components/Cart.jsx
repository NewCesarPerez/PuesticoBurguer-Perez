...
const Cart = () => {
  ...
  return (
    <div className="container my-5 fontFamilyForm">
      ...
      {cart.map((prod) => (
        <div key={prod.id}>
          ...
          <h5 className="text-warning">{prod.title}</h5>
      ...
      <hr />
      <Link to={"/checkout"} className="btn btn-danger mx-2">
        Pagar
      </Link>
      <br />
      <button className="btn btn-warning my-2" onClick={emptyCart}>
        Vaciar carrito
      </button>
    </div>
  );
